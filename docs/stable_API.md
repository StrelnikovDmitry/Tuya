...
to start an application without risk of erasing user's terminal story and to set up the virtual buffer for maximum recource economy, use `tuya_init` at the start of your programm.

The function takes two arguments:
- int buffer_size, which is virtual buffer size. Can be assigned manually or calculated based on terminal size (see **further**)
...
To restore your terminal (basically, revert changes of `tuya_init`), use `tuya_shutdown` function
